  ...
  return (
    <a href={`/seasonaires/${r.id}`}
      ...
      <div className="relative h-48 overflow-hidden">
        ...
        <div className="absolute inset-0" style={{ background: "linear-gradient(to top, rgba(10,27,51,0.5) 0%, transparent 55%)" }} />
        <div className="absolute top-3 right-3 text-xs font-bold px-2.5 py-1" style={{ background: "rgba(10,27,51,0.7)", color: "var(--accent-light)", backdropFilter: "blur(6px)" }}>{r.min_months}+ חודשים</div>
      ...
      <div className="p-5 flex flex-col flex-1 text-right" dir="rtl">
        ...
        <div className="mt-auto flex items-end justify-between">
          <div>
            <span className="font-display text-2xl font-medium" style={{ color: "var(--accent-deep)" }}>€{r.price_per_month.toLocaleString()}</span>
            <span className="text-sm" style={{ color: "var(--stone-soft)" }}> / חודש</span>
          </div>
          <span className="text-sm font-bold group-hover:underline" style={{ color: "var(--accent-deep)" }}>פרטים ←</span>
        </div>
      </div>
    </a>
  );
...
function ComingSoon({ icon, title, desc }: { icon: React.ReactNode; tint?: string; title: string; desc: string }) {
  return (
    <div className="relative p-8 text-center overflow-hidden" style={{ background: "var(--paper)", border: "1px dashed rgba(22,32,46,0.16)" }}>
      <span className="absolute top-4 left-4 text-xs font-bold px-3 py-1 rounded-full" style={{ color: "var(--accent-deep)", background: "var(--accent-wash)" }}>בקרוב · Coming soon</span>
      <div className="inline-flex items-center justify-center w-16 h-16 rounded-full mb-4 border" style={{ borderColor: "var(--accent-line)", color: "var(--accent-deep)" }}>{icon}</div>
  ...
  return (
    <div className="min-h-screen" style={{ background: "var(--ivory)" }} dir="rtl">
      ...
      <section className="relative h-[440px] overflow-hidden">
        ...
        <div className="absolute inset-0" style={{ background: "linear-gradient(to top, rgba(10,27,51,0.94) 0%, rgba(10,27,51,0.5) 55%, rgba(10,27,51,0.6) 100%)" }} />
        <div className="relative max-w-5xl mx-auto px-6 h-full flex flex-col justify-end pb-12" style={{ color: "var(--ivory)" }}>
          ...
          <p className="text-lg mt-4 max-w-2xl font-light" style={{ color: "rgba(247,249,252,0.8)" }}>
      ...
      <section id="board" className="max-w-5xl mx-auto px-6 pt-14 scroll-mt-6">
        ...
        {loading ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {[1,2,3].map(i => <div key={i} className="h-80 animate-pulse" style={{ background: "var(--paper)", border: "1px solid rgba(22,32,46,0.08)" }} />)}
          </div>
        ) : rentals.length === 0 ? (
          <div className="text-center py-16" style={{ background: "var(--paper)", border: "1px solid rgba(22,32,46,0.08)" }}>
      ...
      <section className="max-w-5xl mx-auto px-6 pb-20">
        <div className="text-center relative overflow-hidden p-8 md:p-14" style={{ background: "var(--ink)", color: "var(--ivory)" }}>
          <div className="absolute -left-16 -top-16 w-56 h-56 rounded-full blur-3xl" style={{ background: "var(--accent-wash)" }} />
          ...
          <p className="relative mb-8 max-w-xl mx-auto font-light" style={{ color: "rgba(247,249,252,0.7)" }}>הצטרפו לקבוצת הוואטסאפ של הסיזיונרים, או דברו איתנו ישירות על דירה, סקי פס ועבודה.</p>
          <div className="relative flex flex-col sm:flex-row gap-3 justify-center">
            ...
            <a href={waContact} target="_blank" rel="noopener noreferrer"
              className="btn-ghost font-display px-8 py-4" style={{ color: "var(--accent-light)", borderColor: "var(--accent-line)" }}>
              דברו איתנו
            </a>
          </div>
        </div>
