        ...
        {cells.map((date, i) => {
          ...
          return (
            ...
            >
              ...
              <div className={`relative z-10 ${circle} flex items-center justify-center rounded-full select-none transition-colors font-medium ${!disabled && !isStart && !isEnd ? "hover:bg-[var(--accent-wash)]" : ""}`}
                style={
                  isStart || isEnd
...
const IconMountain = () => (
  <svg width="16" height="14" viewBox="0 0 32 28" fill="none">
    <polygon points="16,2 30,26 2,26" fill="none" stroke="var(--accent-deep)" strokeWidth="2.2" strokeLinejoin="round"/>
    <polygon points="16,10 23,26 9,26" fill="none" stroke="var(--accent-deep)" strokeWidth="1.5" strokeLinejoin="round"/>
  </svg>
);
const IconCal = () => (
...
export default function SearchWidget() {
  ...
  const navBtn = (disabled: boolean, onClick: () => void, children: React.ReactNode) => (
    <button onClick={() => !disabled && onClick()}
      className={`w-9 h-9 flex items-center justify-center rounded-full border transition-colors
        ${disabled ? "border-gray-100 text-gray-300 cursor-not-allowed" : "border-gray-200 text-gray-600 hover:border-[var(--accent)] hover:text-[var(--accent-deep)]"}`}>
      {children}
    </button>
  );
  ...
  return (
    <div ref={ref} className="relative w-full max-w-3xl" dir="rtl">
      ...
      <div className="hidden md:flex items-stretch rounded" style={{ background: "var(--paper)", boxShadow: "0 20px 60px -12px rgba(10,27,51,0.45)" }}>
        ...
        <button onClick={() => openPicker("from")}
          className={`flex-1 px-5 py-5 text-right transition-colors hover:bg-gray-50 ${open && picking === "from" ? "bg-[var(--accent-wash)]" : ""}`}>
        ...
        <button onClick={() => openPicker("to")}
          className={`flex-1 px-5 py-5 text-right transition-colors hover:bg-gray-50 ${open && picking === "to" ? "bg-[var(--accent-wash)]" : ""}`}>
          ...
          {nights && <div className="text-[10px] text-[var(--accent-deep)] font-bold mt-0.5">{nights} לילות</div>}
        </button>
        <div className="w-px bg-gray-100 my-4" />
        <div className="flex items-center px-4 py-5">
          <div>
            ...
            <div className="flex items-center gap-2">
              <button onClick={() => setGuests(g => Math.max(1, g - 1))} className="w-7 h-7 rounded-full border border-gray-200 flex items-center justify-center text-gray-500 hover:border-[var(--accent)] hover:text-[var(--accent-deep)] font-bold text-sm transition-colors">−</button>
              <span className="font-bold text-gray-900 w-5 text-center text-sm">{guests}</span>
              <button onClick={() => setGuests(g => Math.min(12, g + 1))} className="w-7 h-7 rounded-full border border-gray-200 flex items-center justify-center text-gray-500 hover:border-[var(--accent)] hover:text-[var(--accent-deep)] font-bold text-sm transition-colors">+</button>
            </div>
          </div>
        </div>
        <div className="flex items-center px-3">
          <button onClick={handleSearch} className="btn-primary active:scale-95 px-7 py-4 rounded transition-all flex items-center gap-2 text-sm">
      ...
      <div className="md:hidden rounded overflow-hidden" style={{ background: "var(--paper)", boxShadow: "0 20px 60px -12px rgba(10,27,51,0.45)" }}>
        {/* Destination row */}
        ...
        <div className="grid grid-cols-2 border-b border-gray-100">
          <button onClick={() => openPicker("from")}
            className={`px-5 py-3.5 text-right border-l border-gray-100 transition-colors ${open && picking === "from" ? "bg-[var(--accent-wash)]" : "hover:bg-gray-50"}`}>
          ...
          <button onClick={() => openPicker("to")}
            className={`px-5 py-3.5 text-right transition-colors ${open && picking === "to" ? "bg-[var(--accent-wash)]" : "hover:bg-gray-50"}`}>
        ...
        <div className="flex items-center justify-between px-5 py-3">
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              <button onClick={() => setGuests(g => Math.max(1, g - 1))} className="w-8 h-8 rounded-full border border-gray-200 flex items-center justify-center text-gray-500 hover:border-[var(--accent)] hover:text-[var(--accent-deep)] font-bold transition-colors">−</button>
              <span className="font-bold text-gray-900 w-6 text-center">{guests}</span>
              <button onClick={() => setGuests(g => Math.min(12, g + 1))} className="w-8 h-8 rounded-full border border-gray-200 flex items-center justify-center text-gray-500 hover:border-[var(--accent)] hover:text-[var(--accent-deep)] font-bold transition-colors">+</button>
            </div>
          </div>
          <button onClick={handleSearch} className="btn-primary active:scale-95 px-6 py-2.5 rounded transition-all flex items-center gap-2 text-sm">
      ...
      {/* ══ Calendar dropdown ══════════════════════════════════ */}
      {open && (
        <div className="absolute top-full mt-2 rounded shadow-2xl border z-50 p-4 md:p-6" style={{ right: 0, left: 0, background: "var(--paper)", borderColor: "rgba(22,32,46,0.08)" }}>
            ...
            {(["from","to"] as const).map(w => (
              <button key={w} onClick={() => setPicking(w)}
                className={`flex-1 py-2.5 rounded text-xs md:text-sm font-bold transition-all border-2 ${
                  picking === w ? "border-[var(--accent)] bg-[var(--accent-wash)] text-[var(--accent-deep)]" : "border-gray-100 text-gray-500 hover:border-gray-200"
                }`}>
          ...
          <div className="flex items-center justify-between pt-3 border-t border-gray-100 mt-3">
            ...
            {nights
              ? <span className="text-sm font-bold text-[var(--accent-deep)]">{nights} לילות ✓</span>
